...
# Usage: infra_get_value cloudflare CLOUDFLARE_ACCOUNT_ID
infra_get_value() {
    local provider="$1"
    local key="$2"
    local config_file
...
EOF
    ...
    if [[ -f "$config_file" ]]; then
        ...
        echo -e "  Account ID: ${BOLD}${CLOUDFLARE_ACCOUNT_ID:-${CF_ACCOUNT_ID:-not set}}${NC}"
        echo ""
    ...
    if [[ -z "$CLOUDFLARE_ACCOUNT_ID" ]]; then
        read -r -p "Enter your Cloudflare Account ID: " CLOUDFLARE_ACCOUNT_ID
    fi
    ...
    echo ""
    read -r -p "Enter API Token (or press Enter to skip): " CLOUDFLARE_API_TOKEN
...
# Your Cloudflare Account ID (the name wrangler itself reads)
    ...
    for provider in "${MX_INFRA_PROVIDERS[@]}"; do
        ...
        if [[ -f "$config_file" ]]; then
            ...
            case "$provider" in
                cloudflare)
                    local account_id
                    account_id=$(grep -E "^(CLOUDFLARE|CF)_ACCOUNT_ID=" "$config_file" 2>/dev/null | head -1 | cut -d'=' -f2-)
