...
_router_help() {
    ...
    echo "    (override with MX_ROUTER_HTTP_PORT / MX_ROUTER_HTTPS_PORT)."
    ...
    echo "      - traefik.http.routers.\${COMPOSE_PROJECT_NAME}-myapp.rule=Host(\`\${MYAPP_ROUTER_HOST:-myapp.localhost}\`)"
    echo "      - traefik.http.routers.\${COMPOSE_PROJECT_NAME}-myapp.entrypoints=web"
    echo "      - traefik.http.services.\${COMPOSE_PROJECT_NAME}-myapp.loadbalancer.server.port=80"
    echo "      - traefik.docker.network=devmesh-traefik"
    echo ""
    echo "    Traefik keeps ONE router table for the whole machine, so the"
    echo "    router/service/middleware NAMES carry the compose project;"
    echo "    compose resolves COMPOSE_PROJECT_NAME for you. Hostnames keep"
    echo "    their default, and a second stack claims its own with"
    echo "    <SERVICE_UPPER>_ROUTER_HOST=other.localhost make dev."
    echo ""
}
