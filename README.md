...
The **mx router** is a single global Traefik instance that every mx project registers with. Application services publish **no HTTP host ports** — Traefik routes to them by hostname, so two ecosystems' web apps never collide. (Dev overrides do expose backing services like Postgres and Redis to the host for local GUI tools, on a port Docker allocates rather than a pinned one, so a second stack never finds the number taken.) Each service is reachable at a stable hostname:
...
Two copies of the *same* shape coexist as well, with no hand edits. Dev overrides no longer pin host ports: the ports tooling dials on demand (Postgres, Redis, the Node inspector, worker metrics, Vite, Inertia SSR) publish on `0`, so Docker picks a free one per stack and `docker compose -p <project> port db 5432` prints it. Traefik router, service and middleware names carry the compose project, so two stacks that each scaffold an `api` service write two entries into the router table instead of fighting over one.

The one residual, stated plainly: both stacks run, but a hostname belongs to one of them. Default domains are untouched, so the second stack claims its own by exporting a knob rather than editing a shipped file: `API_ROUTER_HOST=api-two.localhost make dev`. Until it does, the second stack is up and healthy but not reachable at `api.localhost`.
...
Four migrations ride with it, all documented at [Upgrade](https://mechcrate.dev/docs/framework/upgrade/):
...
- **Breaking: dev host ports are no longer pinned.** Every dev override used to publish a fixed host port, so the second stack died on `Bind for 0.0.0.0:5432 failed: port is already allocated`. Ports that tooling dials on demand now publish on host port `0`, which Docker allocates free: discover one with `docker compose -p <project> port <service> <container-port>`, or pin it for a session with `DB_HOST_PORT`, `REDIS_HOST_PORT`, `NODE_DEBUG_HOST_PORT`, `METRICS_HOST_PORT`, `VITE_HOST_PORT` or `INERTIA_SSR_HOST_PORT`. Ports a browser dials from a URL it already holds keep today's number as a default and gain an override: `LEPTOS_RELOAD_HOST_PORT` (3001), `ZOLA_LIVERELOAD_PORT` (1024), `NGINX_HTTP_PORT` / `NGINX_HTTPS_PORT`, `TRAEFIK_HTTP_PORT` / `TRAEFIK_HTTPS_PORT` / `TRAEFIK_DASHBOARD_PORT`, `MX_ROUTER_HTTP_PORT` / `MX_ROUTER_HTTPS_PORT`. Port 24678 is gone outright: astro and nuxt serve HMR over the app's own port, and the router upgrades that websocket same-origin, so the publish was holding a port for nothing. What breaks is a script or GUI profile pointed at `localhost:5432` and friends.
- **Breaking: Traefik label names carry the compose project.** `traefik.http.routers.<service>.*` is now `traefik.http.routers.${COMPOSE_PROJECT_NAME}-<service>.*`, and the same for `.services.` and `.middlewares.`. Traefik keeps one router table per machine, so the old unqualified names meant two projects each scaffolding `api` either lost both routers to `Router defined multiple times` or, worse, merged silently into one load-balancer pool serving each other's traffic. Hostnames are unchanged by default; the new per-service override is `<SERVICE_UPPER>_ROUTER_HOST` (`SITE_ROUTER_HOST` for this repo's own site). What breaks is a script that addressed a label or a dashboard router entry by its bare service name.
