...
description: The rules agents work under in this repository — consult the corpus before deciding, evidence before assertion — and where they are published.
sidebar:
  order: 5
---
...
designing an API or implementing a non-obvious algorithm, query the corpus —
`rag_context` with a description of the task, or the narrower `rag_*`
