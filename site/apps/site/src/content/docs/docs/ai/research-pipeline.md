...
1. The top unchecked entry in
   ...
   anyone, human or agent, can append to.
...
commits to a verdict: **NEW** (nothing meaningful, so author a document),
**IMPROVE** (a document covers it, so enumerate its concrete gaps against
current practice), or **FRESH** (covered and current, so log a no-op row and
stop, no pull request). Of the ten runs logged so far one stopped dead at FRESH
with no pull request, and another left one of the two documents it examined
untouched. That is the point: a run is not supposed to always produce a
document.
...
Everything the model contributes itself, inferred patterns and connections it
...
1. `mx rag ingest --dry-run` must report zero warnings, because the document has
   to parse and chunk cleanly before it can be proposed.
