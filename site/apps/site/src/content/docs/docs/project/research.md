...
description: The append-only record of every technique-research run, and the queue of topics waiting for one. Both are published as corpus documents.
sidebar:
  order: 3
---
...
that this project's own lexical retrieval arm was inert. That became
...
The queue. Autonomous runs pop the top unchecked entry; anyone, human or agent,
may append one. Entries carry a one-line reason and who added it:
