...
| `mx self-update` | Update `mx` itself from the release channel. `--check` (exit 10 when newer exists), `--dry-run`, `--yes`, `--to <VERSION>`, `--rollback`, `--from-dir <DIR>`, `--pull` for source checkouts. See [Install](/docs/start/install/) |
