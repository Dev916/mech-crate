...
export function agentInstructions(options: AgentInstructionsOptions = {}): string[] {
  ...
  const lines = [
    ...
    `- **Two mx stacks run at once, and neither localhost ports nor Traefik label names are what they were.** Shipped dev overrides publish no pinned host port. Postgres, Redis, the Node inspector, worker metrics, Vite and Inertia SSR publish on host port \`0\`, so Docker allocates a free one per stack: read it with \`docker compose -p <project> port db 5432\`, or pin it for a session with \`DB_HOST_PORT\`, \`REDIS_HOST_PORT\`, \`NODE_DEBUG_HOST_PORT\`, \`METRICS_HOST_PORT\`, \`VITE_HOST_PORT\`, \`INERTIA_SSR_HOST_PORT\`. Do not tell a user to connect to \`localhost:5432\`, and do not add a pinned publish back to a shipped file; a conformance test refuses it. Browser-facing dev ports keep their number as a default and take an override instead (\`LEPTOS_RELOAD_HOST_PORT\`, \`ZOLA_LIVERELOAD_PORT\`, \`NGINX_HTTP_PORT\`/\`NGINX_HTTPS_PORT\`, \`TRAEFIK_HTTP_PORT\`/\`TRAEFIK_HTTPS_PORT\`/\`TRAEFIK_DASHBOARD_PORT\`, \`MX_ROUTER_HTTP_PORT\`/\`MX_ROUTER_HTTPS_PORT\`). Port 24678 does not exist any more: astro and nuxt serve HMR over the app's own port and the router upgrades it same-origin. Traefik names carry the compose project, so a label is \`traefik.http.routers.\${COMPOSE_PROJECT_NAME}-<service>.*\` (likewise \`.services.\` and \`.middlewares.\`) and a dashboard entry is \`myproj-api@docker\`, not \`api@docker\`. Hostnames are unchanged by default, but a router rule's Host value in a *shipped compose file* is now the interpolation \`\${<SERVICE_UPPER>_ROUTER_HOST:-<domain>}\` rather than a bare hostname: parsing that file for a service URL means resolving the default, not printing it. A running container's labels are already resolved, so \`docker inspect\` and the Traefik dashboard show the real hostname. Two same-shape stacks coexist unaided, but Traefik serves a given hostname to exactly one of them, so a second stack claims its own with \`API_ROUTER_HOST=api-two.localhost make dev\`. Details at ${url('/docs/framework/upgrade/')} and ${url('/docs/framework/router/')}.`,
