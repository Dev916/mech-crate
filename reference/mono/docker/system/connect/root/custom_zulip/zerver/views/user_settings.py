...
def validate_email_change_request(user_profile: UserProfile, new_email: str) -> None:
    raise JsonableError(_("Email changes are disabled by the administrator."))
...
def confirm_email_change(request: HttpRequest, confirmation_key: str) -> HttpResponse:
    ...
    if old_email == "":
        ...
        reset_password_url = generate_password_reset_url(
            user_profile, default_token_generator
        )
...
def json_change_settings(
    ...
    twenty_four_hour_time: Optional[bool] = REQ(
    ...
    starred_message_counts: Optional[bool] = REQ(
        json_validator=check_bool, default=None
    ),
    ...
    display_emoji_reaction_users: Optional[bool] = REQ(
        json_validator=check_bool, default=None
    ),
    ...
    web_stream_unreads_count_display_policy: Optional[int] = REQ(
        json_validator=check_int_in(
            UserProfile.WEB_STREAM_UNREADS_COUNT_DISPLAY_POLICY_CHOICES
        ),
        default=None,
    ),
    ...
    wildcard_mentions_notify: Optional[bool] = REQ(
        json_validator=check_bool, default=None
    ),
    ...
    enable_marketing_emails: Optional[bool] = REQ(
        json_validator=check_bool, default=None
    ),
    ...
    desktop_icon_count_display: Optional[int] = REQ(
        json_validator=check_int_in(UserProfile.DESKTOP_ICON_COUNT_DISPLAY_CHOICES),
        default=None,
    ),
    realm_name_in_email_notifications_policy: Optional[int] = REQ(
        json_validator=check_int_in(
            UserProfile.REALM_NAME_IN_EMAIL_NOTIFICATIONS_POLICY_CHOICES
        ),
        default=None,
    ),
    ...
    email_address_visibility: Optional[int] = REQ(
        json_validator=check_int_in(UserProfile.EMAIL_ADDRESS_VISIBILITY_TYPES),
        default=None,
    ),
) -> HttpResponse:
    if (
    ...
    if new_password is not None:
        raise JsonableError(_("Password changes are disabled by the administrator."))
        ...
        try:
            ...
            raise JsonableError(
                _(
                    "You're making too many attempts! Try again in {seconds} seconds."
                ).format(seconds=secs_to_freedom),
            )
    ...
    request_settings = {
        k: v for k, v in locals().items() if k in user_profile.property_types
    }
    ...
    if timezone is not None and user_profile.timezone != timezone:
        do_change_user_setting(
            user_profile, "timezone", timezone, acting_user=user_profile
        )
...
def set_avatar_backend(request: HttpRequest, user_profile: UserProfile) -> HttpResponse:
    ...
    if user_file.size > settings.MAX_AVATAR_FILE_SIZE_MIB * 1024 * 1024:
        raise JsonableError(
            _(
                "Uploaded file is larger than the allowed limit of {max_size} MiB"
            ).format(
