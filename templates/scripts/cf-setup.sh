...
#
# CLOUDFLARE_ACCOUNT_ID is the canonical name (wrangler reads it straight from
# the environment); CF_ACCOUNT_ID is the deprecated alias this script used to
# write, still accepted so an older project keeps working.
if [[ -f "$CF_ENV_FILE" ]]; then
    # shellcheck source=/dev/null
    source "$CF_ENV_FILE"
    CLOUDFLARE_ACCOUNT_ID="${CLOUDFLARE_ACCOUNT_ID:-${CF_ACCOUNT_ID:-}}"
    echo -e "Current configuration:"
    echo -e "  Account ID: ${BOLD}${CLOUDFLARE_ACCOUNT_ID:-not set}${NC}"
    if grep -q '^[[:space:]]*CF_ACCOUNT_ID=' "$CF_ENV_FILE"; then
        warn "CF_ACCOUNT_ID is deprecated. Reconfiguring rewrites it as CLOUDFLARE_ACCOUNT_ID."
    fi
    echo ""
...
if [[ -z "${CLOUDFLARE_ACCOUNT_ID:-}" ]]; then
    error "Account ID is required. Or run 'mx infra setup cloudflare' to configure credentials globally."
fi
...
#
# This file is sourced by make/cloudflare.mk, which reads it AFTER the global
# config (~/.mech-crate/config/infra/cloudflare.env), so these values win.
