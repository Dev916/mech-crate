...
|------|-------------|----------|
| **Regular Worker** | Standard edge worker with fetch handler | APIs, proxies, static sites |
| **Cron Worker** | Scheduled worker with cron triggers | Background jobs, data sync, cleanup |
| **Container Worker** | Worker backed by Docker container | SSR apps, complex backends, stateful services |
...
```bash
# Interactive mode - choose worker type
...
The interactive wizard will walk you through configuring:
...
**Container Worker:**
- Container port
- Sleep timeout (auto-shutdown)
- Max instances (dev/prod)
- Custom domain
- Dockerfile template
...
Standard edge workers that handle HTTP requests with minimal latency.

```bash
make cf-init a=api.example.com type=worker
```

Features:
- Fetch handler for HTTP requests
- KV, R2, D1 bindings support
- Environment variables
- Custom domain routing
...
Scheduled workers that run on a cron schedule.

```bash
make cf-init a=sync-job type=cron
```

Features:
- Configurable cron schedules (e.g., `*/5 * * * *`)
...
Example endpoints generated:
- `/_health` - Health check with last run status
- `/trigger` - POST to manually trigger the job

### Container Workers

Workers backed by Docker containers using Durable Objects.

```bash
make cf-init a=app.example.com type=container
```

Features:
- Full Docker container support
- Auto-scaling with configurable limits
- Sleep timeout (container shuts down after inactivity)
- Container lifecycle management endpoints
- Dockerfile template generation

Example endpoints generated:
- `/_health`, `/_healthz`, `/_readyz` - Health checks
- `/_container/status` - Container state
- `/_container/start` - Start container
- `/_container/stop` - Stop container
- `/_container/restart` - Restart container
...
| Command | Description |
|---------|-------------|
| `make cf-init a=<app>` | Initialize app (interactive) |
| `make cf-init a=<app> type=worker` | Initialize regular worker |
| `make cf-init a=<app> type=cron` | Initialize cron worker |
| `make cf-init a=<app> type=container` | Initialize container worker |
