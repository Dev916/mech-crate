...
- Claude Code enables tool search by default on Claude 4.5-generation and later models; a request that needs a server's tools triggers a `ToolSearch` call and matching tools are fetched on demand. A discovery cache that lets servers connect on first use needs v2.1.221 or later and, since v2.1.238, is off by default unless a gradual rollout has enabled it (`MCP_DISCOVERY_CACHE=1` turns it on) [6].
- The Claude API takes `defer_loading: true` per tool (for MCP-connector tools it is set once on the `mcp_toolset` entry's `default_config`, or per tool in its `configs`, not on the server's own definitions) plus a `tool_search_tool` (regex or BM25 variant); deferred tools stay out of the prompt prefix so prompt caching survives, and the docs advise keeping the 3-5 most-used tools non-deferred [4].
...
| Namespacing | Prefix by service or resource (`asana_projects_search`); unambiguous parameter names (`user_id`, not `user`) | A shared prefix lets one search match the whole group [4]; Anthropic found prefix-vs-suffix placement had non-trivial effects on its tool-use evaluations [8] |
| Descriptions | Write as if onboarding a new hire: make query formats and terminology explicit; include the words users say | Anthropic reports that small description refinements yielded dramatic improvements on its held-out Slack and Asana evaluations (the post shows the curves but gives no percentage); tool search indexes names, descriptions, argument names and argument descriptions [4][8] |
| Input schema | Valid JSON Schema object; no-parameter tools use `{ "type": "object", "additionalProperties": false }`; strict, typed inputs | Spec MUST (a valid JSON Schema object, not null); the empty-object shape is the spec's recommended form for no-parameter tools; strict data models avoid ambiguity [3][8] |
...
| Annotations | Set `readOnlyHint`/`destructiveHint`/`idempotentHint`/`openWorldHint`, but never rely on them for safety | Clients MUST treat annotations as untrusted unless they come from trusted servers; enforce approval in the host policy (Neon: `_meta` flag) [3][11] |
...
| Security | Validate all inputs, implement access control, rate-limit, sanitise outputs; never mark secrets with `x-mcp-header` | The first four are spec MUSTs for servers; the `x-mcp-header` rule is a SHOULD NOT [3] |
