...
**Bennett's argument against a service layer** is the strongest statement of the model-first position [7]. Its load-bearing claims: the models, plus "associated utility code, like custom `Manager` or `QuerySet` subclasses", "are the API exposed to other code", and therefore "are the place where the 'business logic' should be implemented"; swapping the data layer for something else "happens almost never" in his experience, so indirection built for that purpose never pays; and adopting a service layer means "whatever your business was previously, now your business is that plus developing and maintaining something close to your own private ORM" [7]. He also notes the collateral damage, in two separate parts. The integrations: "no more handy generic views, no more helpful auto-generating forms or (if you're using DRF) serializers, and so on, because those all break the “service” abstraction by exposing other code to the existence and APIs of the underlying ORM" [7]. And separately `QuerySet` itself, a "caching query wrapper with a bunch of functionality to turn common use patterns into efficient query behavior", whose benefit disappears once you abstract its existence away: "you either lose that or have to reimplement it yourself" [7]. His closing advice to anyone committed to the pattern is to consider a Data Mapper ORM such as SQLAlchemy instead of fighting Django [7].
...
**Push work into the database where 5.0 and later let you.** `GeneratedField`, added in 5.0 and described there as allowing "creation of database generated columns" [46], computes a column in the database; its expression "should be deterministic and only reference fields within the model (in the same database table)", generated fields cannot reference other generated fields, and PostgreSQL before 18 only supports persisted columns while Oracle before 23ai only supports virtual ones [15]. `Field.db_default`, also 5.0, "sets a database-computed default value" [46] that "will be used when inserting rows outside of the ORM or when adding a new field in a migration", while a Python `default` still wins for instances created in Python [15]. `CompositePrimaryKey`, added in 5.2 [4], "must be defined as the model's `pk` attribute" [15]. Constraints are the Styleguide's first recommendation for validation, on the stated grounds of less code to write and less code to maintain [6].
...
Adding a unique, non-nullable column to a populated table cannot be one migration: "Applying a “plain” migration that adds a unique non-nullable field to a table with existing rows will raise an error because the value used to populate existing rows is generated only once, thus breaking the unique constraint" [17]. Django's documented recipe is three migrations [17]:

1. `0004_add_uuid_field`: the auto-generated `AddField`, edited to change `unique=True` to `null=True`, which "will create the intermediary null field and defer creating the unique constraint until we've populated unique values on all the rows" [17].
2. `0005_populate_uuid_values`: an empty migration carrying a `RunPython` or `RunSQL` operation "to generate a unique value (UUID in the example) for each existing row" [17].
3. `0006_remove_uuid_null`: the same operation copied across with `AddField` changed to `AlterField`, restoring `unique=True` [17].

The recipe's own warning is about data in flight, not about deploy ordering: "Note there is a race condition if you allow objects to be created while this migration is running. Objects created after the `AddField` and before `RunPython` will have their original `uuid`'s overwritten" [17].

**Batching and `atomic = False` are a separate technique**, documented in that howto's own non-atomic-migrations section rather than as part of the unique-field recipe. On SQLite and PostgreSQL "migrations will run inside a transaction by default", and "For use cases such as performing data migrations on large tables, you may want to prevent a migration from running in a transaction by setting the `atomic` attribute to `False`" [17]. Combine the two when the backfill is large enough that holding one transaction is itself the problem [17].
...
**On deploy ordering around a backfill.** Django's unique-field recipe warns only that rows created between the `AddField` and the `RunPython` get overwritten; it says nothing about when to ship the code that writes the new column. My own rule, inferred rather than documented: keep the writing code dark until the backfill migration has finished everywhere, because the alternative is either overwritten values or a unique-constraint failure at the `AlterField` step. Guard it with a flag rather than with deploy timing, since a rollback re-exposes the old code path.
